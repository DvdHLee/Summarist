...
function SignupModal() {
    ...
    return (
        <>
            <Modal open={openModal === "signup"} onClose={() => dispatch(closeModal())} className='flex justify-center items-center border border-none'>
                <div className='w-[90%] h-fit bg-white md:w-[400px] rounded-lg flex flex-col items-center outline-none relative'>
                    <div className='flex flex-col items-center pt-[48px] px-[32px] pb-[24px] w-full'>
                        ...
                        className='h-10 border border-[2px] rounded border-[#bac8ce] text-[#394547] w-full px-3 mb-4 text-sm focus:border-[#2bd97c] outline-none'></input>
                        <button className='flex bg-[#2bd97c] h-10 relative text-white w-full rounded justify-center items-center hover:bg-[#20ba68]' onClick={handleSignup}>
                            <h2 className='text-[16px] text-black'>Signup</h2>
                        </button>
                    </div>
                    <button className='text-[#116be9] text-[14px] font-light mb-4' onClick={clickedOpenLogin}>Already have an account?</button>
